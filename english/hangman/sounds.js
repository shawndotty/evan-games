class SoundManager {
    ...
    win() {
        ...
        this.playTone(1046.50, 'triangle', 0.8, now + speed*3);// C6
    }
    ...
    lose() {
        ...
        const now = 0;
        const speed = 0.3;
        this.playTone(392.00, 'triangle', 0.4, now);           // G4
        ...
        osc.frequency.linearRampToValueAtTime(250, this.ctx.currentTime + speed*3 + 1.0); // Slide down

        gain.gain.setValueAtTime(0.3, this.ctx.currentTime + speed*3);
        gain.gain.linearRampToValueAtTime(0.01, this.ctx.currentTime + speed*3 + 1.0);
